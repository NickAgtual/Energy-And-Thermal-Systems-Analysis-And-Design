...
% Fin pitch
% 39.3701 in/meter
HX.finPitch = 9.05 * 39.3701; % Fin / meter
...
pressureDrop = ((air.G .^ 2) * (1 / air.density) / 2) * ...
    ((1 + HXair.sigma .^ 2) * ((air.specificVol2 / (1 / ...
    air.density)) -1) + (air.f .* (HX.Ltm ./ HXair.rh) * ...
    (air.meanTemp / air.inletTemp))); % Pa
...
verificationAir.eta = ones(1, length(HX.Ltm)) * .93;
...
% Plotting pressure drop vs. length
yyaxis left
plot(HX.Ltm, pressureDrop, 'DisplayName', 'Pressure Drop')
ylabel('\emph {Pressure Drop (Pa)}','fontsize',14,'Interpreter', 'latex');

% effectiveness vs. length
yyaxis right
plot(HX.Ltm, verification.epsilon, 'DisplayName', 'Effectiveness')
ylabel('\emph {Effectiveness ($\epsilon$)}','fontsize',14, ...
    'Interpreter','latex');
...
% Plot descriptors
xlabel('\emph {HX Length (m)}','fontsize',14,...
    'Interpreter', 'latex');
title('\emph {Effect of Heat Exchanger Geometry}',...
